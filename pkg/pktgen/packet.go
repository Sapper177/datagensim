...
type Payload struct {
	Size int
	Data []byte // TODO: implement payload data
}
...
func buildPayload(payloadSize int, payload []byte) []byte {
	payload_buf := make([]byte, payloadSize)
	for i := range payload_buf {
...
func BuildPacket(c *PacketConfig) ([]byte, error) {
	...
	// payload := make([]byte, c.Payload.Size)
	// Optionally, fill the payload with data
	layersToSerialize = append(layersToSerialize, gopacket.Payload(c.Payload.Data))
